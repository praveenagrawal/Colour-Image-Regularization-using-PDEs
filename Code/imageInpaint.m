function [ imgOutput ] = imageInpaint( imgInput, mask )
    ...
    padIy2 = padIy.*padIy;
    padIxIy = padIx.*padIy;
    weight = fspecial('gaussian', [windowSize windowSize], 1.5);
    %weight = 1;
    ...
    for i = 1:1:rows
        for j = 1:1:columns
            if(mask(i,j,:)>0)
                ...
                %f2 = 1; %f1 = 0;
                f2 = 1;%/(sqrt(1+eigenvalue(1,1)+eigenvalue(2,2)));
                f1 = 1/((1+eigenvalue(1,1)+eigenvalue(2,2))^1.2);
